...
# ---------------------------------
def blofin_api(path, method, paramsMap):
    try:
        APIURL = "https://openapi.blofin.com"
        APIKEY = credenciales['api_key']
        SECRETKEY = credenciales['api_secret']
        ...
        print(f"\nERROR LLAMANDO LA API DE BLOFIN\n{e}")
# ---------------------------------
...
def precio_actual_activo(symbol):
    try:
        ...
        result = blofin_api(path=path, method=method, paramsMap=paramsMap)

        precio = float(result['data'][0]['last'])
        return precio
...
def apalancameinto_max(symbol):
    try:
        ...
        return  int(blofin_api(path, method, paramsMap)['data'][0]['maxLeverage'])
    
    except Exception as e:
        print(f"\nERROR BUSCANDO EL APALANCAMIENTO MÁXIMO DE {symbol} EN BLOFIN\n{e}")
# ------------------------------------------------------
...
def nueva_orden(symbol, order_type, quantity, price, side, leverage):
    try:
        ...
        apalancamiento = blofin_api(path=path, method=method, paramsMap=paramsMap)
        ...
        order = blofin_api(path=path, method=method, paramsMap=paramsMap)
        if order['code'] == 0:
...
def cancelar_ordenes(symbol):
    try:
        ...
        ordenes = blofin_api(path=path, method=method, paramsMap=paramsMap)
...
def obtener_ordenes(symbol):
    try:
        ...
        oredenes_abiertas = blofin_api(path=path, method=method, paramsMap=paramsMap)
...
def cancelar_orden(symbol, orderId):
    try:
        ...
        orden = blofin_api(path=path, method=method, paramsMap=paramsMap)
        if orden['code'] == 0:
...
def cerrar_posicion(symbol, positionSide):
    try:
        ...
        print(json.dumps(blofin_api(path=path, method=method, paramsMap=paramsMap)),indent=2)
...
#orden = precio_actual_activo("BTC-USDT")
orden = apalancameinto_max("BTC-USDT")
