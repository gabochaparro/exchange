...
def cargar_json():
    ...
    try:
        ...
        if archivo:
            ...
            ventana.title(f"Classic Grid - {nombre_archivo}")
...
def actualizar_interfaz():
    ...
    for i, (clave, valor) in enumerate(data.items()):
        ...
        if isinstance(valor, bool):  # Crear un botón toggle para valores booleanos
            ...
            if nombre_archivo == parametros_iniciales:
...
def toggle_boolean(clave):
    if clave != "inverso" or nombre_archivo == parametros_iniciales:
...
parametros_iniciales = "parametros_iniciales.json"
ventana = tk.Tk()
ventana.title(f"Classic Grid")
