...
import infinity_20_remoto

try:
    # Directorios locales
    directorio_credenciales = "future/estrategias/infinity/credenciales.json"
    parametros_iniciales = "future/estrategias/infinity/parametros_infinity_2.0.json"
    carpeta_parametros = "/future/estrategias/infinity/parametros/*"
    carpeta_salida = "/future/estrategias/infinity/salida/*"
    ruta_infinity_servidor = "/infinity_20_remoto"
    ...
    def correr_infinity():
        try:
            ...
            if opcion_seleccionada.get() == "local":
                subprocess.run(["python3", "future/estrategias/infinity/infinity_2.0.py"])
            else:
                threading.Thread(target=infinity_20_remoto.abrir_conexion_ec2_y_ejecutar, daemon=True).start()
