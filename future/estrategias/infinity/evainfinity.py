def iniciar():
    ...
    # VARIABLES GLOBALES
    global parametros_copia, balance_inicial, cuenta, precio_actual, grid, beneficio_max, apalancamiento, riesgo_max, ordenes_abiertas, posiciones, parametros, parejas_compra_venta, parejas_compra_venta_short, pausa, tendencia, margen_disponible, iniciar_estrategia

    # GESTIONAR ARCHIVOS DE PARAMETROS Y DE SALIDA
    # --------------------------------------------
    # Abrir el archivo parametros.json y cargar su contenido
    parametros = json.load(open("parametros_iniciales.json", "r"))

    # Especifica la ruta de la carpeta parametros
    carpeta_parametros = 'parametros/*'
    ...
    for archivo in archivos:
        try:
            ...
            print(f"\n{archivo} es una carpeta, omitiendo.")
        except FileNotFoundError:
            print(f"\n{archivo} no existe.")
        except PermissionError:
            print(f"\nNo tienes permisos para eliminar {archivo}.")
    ...
    if parametros['inverso']:
        parametros_copia = f"parametros/{parametros['activo'].upper()}_{parametros['exchange'].upper()}_INVERSO_{fecha_inicio}.json"
    else:
        parametros_copia = f"parametros/{parametros['activo'].upper()}_{parametros['exchange'].upper()}_LINEAL_{fecha_inicio}.json"
    shutil.copy("parametros_iniciales.json", parametros_copia)
    # --------------------------------------------
    ...
    def verificar_parametros():
        try:
            ...
            if inverso and exchange != "BYBIT":
                print("Inversión inicial:", round(cuenta*apalancamiento,8), activo)
            else:
            ...
            print("ERROR EN LA FUNCIÓN verificar_parametros()")
            print(e)
            print("")
            exit()
    # -----------------------------------------
                        ...
                        if orden['orderId'] == pareja['compra']['orderId']:
                            ...
                            if exchange.upper() == "BINANCE":
                                ...
                                if orden['status'] == "FILLED" and pareja['compra']['ejecutada'] == False:
                                    if inverso:
                                        cantidad = orden['executedQty']
                                    else:
                                        cantidad = orden['origQty']
                                    pareja['compra']['ejecutada'] = True
                                    pareja['compra']['fecha_ejecucion'] = orden['time']
                                    pareja['compra']['cantidad'] = str(cantidad)
                                    pareja['compra']['price'] = str(orden['avgPrice'])
                                    pareja['compra']['monto'] = str(float(pareja['compra']['cantidad'])*float(pareja['compra']['price']))
                                    #print(json.dumps(parejas_compra_venta,indent=2))
                            ...
                            if exchange.upper() == "BINANCE":
                                ...
                                if orden['status'] == "FILLED" and pareja['venta']['ejecutada'] == False:
                                    if inverso:
                                        ...
                                        cantidad = orden['origQty']
                                    pareja['venta']['ejecutada'] = True
                                    pareja['venta']['fecha_ejecucion'] = orden['time']
                                    pareja['venta']['cantidad'] = str(cantidad)
                                    pareja['venta']['price'] = str(orden['avgPrice'])
                                    pareja['venta']['monto'] = str(float(pareja['venta']['cantidad'])*float(pareja['venta']['price']))
                                    if inverso:
    ...
    def limpiar_parejas_long():
        try:
            ...
            print("ERROR EN LA FUNCIÓN limpiar_parejas_long()")
            print(e)
            print("")
    # --------------------------------------------
    ...
    def limpiar_parejas_short():
        try:
            ...
            print("ERROR EN LA FUNCIÓN limpiar_parejas_short()")
            print(e)
            print("")
    # --------------------------------------------
                    ...
                    if not(orden_compra_puesta):
                        if precio_actual > prox_compra:
                            ...
                            if orden_compra != None:
                                time.sleep(3.06)
                                if inverso and exchange != "BINANCE":
                                    cantidad = lote*float(orden_compra['qty'])/float(orden_compra['price'])
                                else:
                                    cantidad = float(orden_compra['qty'])
                                parejas_compra_venta.insert(0,{
                                                            ...
                                                            "compra": {
                                                                        ...
                                                                        "monto": str(float(cantidad)*float(orden_compra['price'])),
                                                                        "ejecutada": False,
                                                                        "fecha_ejecucion" : "-"
                                                                        },
                                                            "venta": {
                        ...
                        if precio_actual < prox_venta:
                            ...
                            if orden_venta != None:
                                ...
                                if inverso and exchange != "BINANCE":
                                    ...
                                    cantidad = float(orden_venta['qty'])
        ...
        try:
            ...
            if tipo == "" or "LONG":
                if margen_disponible < cantidad_usdt:
                    ...
                    if  pendiente > 1 and pareja_eliminar != {}:
                        
                        # Cancelar orden mas baja
                        print("\nCancelar orden por falta de margen")
                        if inverso:
    ...
    def mostrar_lista(data):
        try:
            ...
            if data == parejas_compra_venta:
                if inverso:
                    ruta = f'salida/{activo}_{exchange}_INVERSO_{fecha_inicio}_LONG.json'
                else:
                    ruta = f'salida/{activo}_{exchange}_LINEAL_{fecha_inicio}_LONG.json'
                json.dump(salida, open(ruta, "w"), indent=4)
            
            if data == parejas_compra_venta_short:
                if inverso:
                    ruta = f'salida/{activo}_{exchange}_INVERSO_{fecha_inicio}_SHORT.json'
                else:
                    ruta = f'salida/{activo}_{exchange}_LINEAL_{fecha_inicio}_SHORT.json'
                json.dump(salida, open(ruta, "w"), indent=4)        
    ...
    def detener_estrategia():
        try:
            ...
            # Variables globales
            global iniciar_estrategia, riesgo_max, beneficio_max, pausa
        ...
        try:
            ...
            while iniciar_estrategia:
                try:
                    ...
                    if exchange == "BYBIT":
                        if inverso:
                            ema_9 = bybit_inverse.obtener_ema(symbol=symbol+"USD", interval=5, periodo=9)
                            ema_54 = bybit_inverse.obtener_ema(symbol=symbol+"USD", interval=5, periodo=54)
                        else:
                            ema_9 = bybit.obtener_ema(symbol=symbol+"USDT", interval=5, periodo=9)
                            ema_54 = bybit.obtener_ema(symbol=symbol+"USDT", interval=5, periodo=54)

                    if exchange == "BINANCE":
                        if inverso:
                            ema_9 = binance_inverse.obtener_ema(symbol=symbol+"USD_PERP", interval="5m", periodo=9)
                            ema_54 = binance_inverse.obtener_ema(symbol=symbol+"USD_PERP", interval="5m", periodo=54)
                        else:
                            ema_9 = binance_.obtener_ema(symbol=symbol+"USDT", interval=5, periodo=9)
                            ema_54 = binance_.obtener_ema(symbol=symbol+"USDT", interval=5, periodo=54)
                    ...
                    print(f"ERROR EN detectar_tendencia() \n{e}")
        except Exception as e:
            print(f"\nERROR EN LA FUNCIÓN detectar_tendencia() \n{e}")
            time.sleep(2.7)
    # ---------------------------------------------
    ...
    def auxiliar():
        try:
            global precio_actual, ordenes_abiertas, posiciones, cuenta, apalancamiento, parametros, parejas_compra_venta, parejas_compra_venta_short, pausa
            ...
            while iniciar_estrategia:
                ...
                if inverso:
                    ...
                    if precio_actual == 0 or precio_actual == None:
                        precio_actual = inverse.precio_actual_activo(exchange=exchange, symbol=activo)
                else:
                ...
                if exchange == "BINANCE":
                    if inverso:
                        if apalancamiento != inverse.apalancamiento_actual(exchange, activo):
                            inverse.apalancamiento(exchange, activo, apalancamiento)
                    else:
                        if apalancamiento != future.apalancamiento_actual(exchange, activo):
                            future.apalancamiento(exchange, activo, apalancamiento)
            ...
            print(f"ERROR EN LA FUNCIÓN auxiliar() \n{e}")
            time.sleep(3)
    #--------------------------------------------
    ...
    # Balance inicial
    if inverso and exchange != "BYBIT":
        balance_inicial = inverse.patrimonio(exchange=exchange,symbol=activo)*precio_actual
    else:
    ...
    # Iniciar estrategia
    iniciar_estrategia = verificar_parametros()
