...
#  -----------------------------------------------
try:
    import asyncio, json, logging, sys, traceback, numpy as np, time
...
try:
    # Configurar logging
    def configurar_logging(archivo: str = "salida/log/app.log", debug: bool = False):
        try:
            # 1️⃣ Crea un logger
...
async def precio_actual_ws(channel_type, symbol):
    ...
    def callback(message):
        global precio_actual
        precio_actual = float(message['data'][0]['p'])
    ws = WebSocket(testnet=False, channel_type=channel_type)
    ws.trade_stream(symbol=symbol.upper(), callback=callback)
    ws.ping_interval = 20
    ws.ping_timeout = 10
    while True:
...
def apalancameinto(category, symbol, leverage):
    try:
        ...
        logger.error(f"Error en apalancameinto() {traceback.format_exc()}")
# ------------------------------------------------------
...
def nueva_orden(category, symbol, order_type, quantity, price, side, leverage, tp=0.0, sl=0.0):
    try:
        ...
        logger.error(f"Error en nueva_orden() {traceback.format_exc()}")
# ---------------------------------------------------
...
def cancelar_orden(category, symbol, orderId):
    try:
        ...
        logger.error(f"Error en cancelar_orden() {traceback.format_exc()}")
# -----------------------------
...
# ------------------------------------
def obtener_ema(category: str, symbol: str, interval: str = "1", periodo: int = 9, vela: int = 1) -> float:
    """
    ...
    try:
        ...
        kl = bybit_session.get_kline(category=category, symbol=symbol, interval=interval, limit=limit)
        ...
        logger.error(f"Error obteniendo EMA: {e}")
        return None
# ------------------------------------
...
def actualizar_grid(category, symbol, precio_actual, distancia_grid, precio_referencia, comision):
    try:
        ...
        # Generar nuevo grid por cmbio de precio de referencia
        ...
        distancia_grid = distancia_grid + 2*comision
...
async def estrategia():
    global porc_long_actual, porc_short_actual, distancia_grid_actual, precio_referencia_actual, ordenes_abiertas
    riesgo = False
    while True:
        try:
            # Parametros
            try:
                parametros = json.load(open("parametros.json", "r"))
                ...
                logger.error(f"Error leyendo los parametros {traceback.format_exc()}")
                logger.info("Leyendo parametros nuevamente...")
                return
            ...
            if ((100*(patrimonio_actual-balance_inicial)/balance_inicial <= -abs(max_perdida) and max_perdida != 0) or 
                ...
                ordenes_abiertas = obtener_ordenes(category, symbol)
                for orden in ordenes_abiertas:
                    if orden['positionIdx'] == 2:
                        cancelar_orden(category, symbol, orderId=orden['orderId'])
                logger.info("ESTRATEGIA LONG DETENIDA POR GESTION DE RIESGO")
                json.dump(parametros, open("parametros_iniciales.json", "w"))

            # Cancelar ordenes por pausa o cambio de parametros
            if pausa or porc_long != porc_long_actual or porc_short != porc_short_actual or distancia_grid != distancia_grid_actual or precio_referencia != precio_referencia_actual:
                for orden in ordenes_abiertas:
            ...
            # Verificación de Emas y cancelar ordenes contrarias
            if proteccion_ema954:
                ema9 = obtener_ema(category, symbol, interval="5", periodo=9)
                ema54 = obtener_ema(category, symbol, interval="5", periodo=54)
                if ema9 > ema54 or direccion == "LONG":
                    pasusa_long = False
                    pasusa_short = True
                if ema9 < ema54 or direccion == "SHORT":
                    pasusa_long = True
                    pasusa_short = False
            else:
                pasusa_long = False
            ...
            if direccion == "LONG" or pasusa_short:
                for orden in ordenes_abiertas:
                    if orden['positionIdx'] == 2 and not orden['reduceOnly']:
                        cancelar_orden(category, symbol, orden['orderId'])
            if direccion == "SHORT" or pasusa_long:
                for orden in ordenes_abiertas:
                    if orden['positionIdx'] == 1 and not orden['reduceOnly']:
                        cancelar_orden(category, symbol, orden['orderId'])

            # Cambiar apalancamiento
            if str(leverage) != obtener_posicion(category, symbol)[0]['leverage']:
            ...
            # Cantidad por cada compra y venta
            ...
            if not riesgo and not pausa:
                ...
                for nivel in grid:
                    ...
                    if 0 != grid.index(nivel) != len(grid)-1:
                        ...
                        if not pasusa_long and (direccion == "LONG" or direccion == "RANGO"):
                        ...
                        if  not pasusa_short and (direccion == "SHORT" or direccion == "RANGO"):
            ...
            logger.error(f"Error en on_change(): {traceback.format_exc()}")
...
while True:
    try:
        ...
        logger.error("Error leyendo los parametros iniciales, reintentando...")
        time.sleep(3.6)
#  -----------------------------------------------
...
#  -----------------------------------------------
async def main():
    await asyncio.gather(patrimonio(category, symbol), precio_actual_ws(category, symbol), estrategia(), return_exceptions=True)
#  -----------------------------------------------
