...
parametros = json.load(open("spot/estrategias/infinity/parametros_infinity_2.0.json", "r"))
...
cuenta = spot.patrimonio(exchange=exchange)                                                       # Inversión de la estrategia
...
def parametros():
    try:
        # Variables globales
        global multiplo, cantidad_usdt, cantidad_usdt_short
        ...
        print("Precio de referencia:", referencia, "USDT")
        print("Cantidad de cada compra:", round(cantidad_usdt,2), "USDT")
        print(f"Distancia entre cada grid: {ganancia_grid-0.11}%")
        print(f"Ganancas del grid: {ganancia_grid_long}%")
                        ...
                        if exchange.upper() == "BYBIT":
                            ...
                            if orden['orderStatus'] == "Filled" and pareja['venta']['ejecutada'] == False:
                                ...
                                if pareja['sl']['orderId'] != "":
                                    spot.cancelar_orden(exchange, activo, orderId=pareja['sl']['orderId'])
                                    print("SL Cancelado.")
                            
                            # Obtener la orden de sl
                            if pareja['sl']['orderId'] != "":
                                ordenes_sl = spot.obtener_ordenes(exchange=exchange, symbol=symbol, orderId=pareja['sl']['orderId'])
                                ...
                                if ordenes_sl != None and ordenes_sl != []:                         
                                    if ordenes_sl[0]['orderStatus'] == "Filled" and ordenes_venta[0]['orderStatus'] == "Untriggered":
                                        ...
                                        if pareja['venta']['orderId'] != "":
                                            spot.cancelar_orden(exchange, activo, orderId=pareja['venta']['orderId'])
                                            print("TP Cancelado.")
                                            print("")
    ...
    try:
        ...
        while iniciar_estrategia:
                ...
                if not(orden_compra_puesta):
                    if spot.margen_disponible(exchange)*apalancamiento > cantidad_usdt and precio_actual > prox_compra:
                        orden_compra = spot.nueva_orden(exchange=exchange, symbol=symbol, order_type="LIMIT", quantity=qty, price=prox_compra, side="BUY", leverage=apalancamiento)
                ...
                if not(orden_condicional_compra_puesta) and condicional_long:
                    if spot.margen_disponible(exchange)*apalancamiento > cantidad_usdt and precio_actual < prox_venta:
                        orden_compra = spot.nueva_orden(exchange=exchange, symbol=symbol, order_type="CONDITIONAL", quantity=qty, price=prox_venta, side="BUY", leverage=apalancamiento)
                        if orden_compra != None:
                            parejas_compra_venta.insert(0,{
                                                        "general": {
                                                                    "fecha": datetime.now().strftime("%Y-%m-%d - %I:%M:%S %p"),
                ...
                for compra_venta in parejas_compra_venta:
                    if compra_venta["compra"]['ejecutada'] and compra_venta["venta"]['orderId'] == "":
                        ...
                        if precio_actual < compra_venta["venta"]['price']:
                            orden = spot.take_profit(exchange=exchange, symbol=symbol, positionSide="LONG", stopPrice=compra_venta["venta"]['price'], type="LIMIT",tpSize=str(compra_venta["venta"]['cantidad']))
                        else:
                            orden = spot.take_profit(exchange=exchange, symbol=symbol, positionSide="LONG", stopPrice=compra_venta["venta"]['price'], type="MARKET",tpSize=str(compra_venta["venta"]['cantidad']))
...
def margen():
    try:
        ...
        if spot.margen_disponible(exchange)*apalancamiento < cantidad_usdt:
                ...
                for pareja in parejas_compra_venta:
                    ...
                    # Buscar le mas bajo
                    if not(pareja['compra']['ejecutada']) and abs(pareja['compra']['price']-spot_ws.precio_actual) > max_distancia:
                        max_distancia = abs(pareja['compra']['price']-spot_ws.precio_actual)
                ...
                if  pendiente > 1:
                    ...
                    spot.cancelar_orden(exchange, activo, pareja_eliminar['compra']['orderId'])
...
def mostrar_lista(data):
    try:
        ...
        draw.text((180, 50), f"{round(spot.patrimonio(exchange),2)} USDT", font=font, fill=text_color)
...
def ganancia_actual():
    try:
        return 100*(spot.patrimonio(exchange) - balance_inicial)/balance_inicial
...
# Balance inicial
balance_inicial = spot.patrimonio(exchange)
